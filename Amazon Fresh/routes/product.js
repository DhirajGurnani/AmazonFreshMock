...
exports.getImageByImageUrl = function(request, response) {
	try {
		mongo.connect(mongoURL, function() {
			...
			mongoDbHelper.doesExistInDb(images, {
...
exports.getProductBySearch = function(request, response) {	
	try {
		if(true) {
			if(true) {
				var product_name = request.body.product_name;  // value is got from the url api/product/:pid
				var sqlQuery = sqlQueryList.getgetProductBySearch(product_name);
				dbHelper.executeQuery(
						sqlQuery, 
						function(rows) {
							response.send({
